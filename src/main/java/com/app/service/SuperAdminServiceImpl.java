...
public class SuperAdminServiceImpl implements ISuperAdminService{
	...
	@Override
	public List<?> revenueReport(String busName, String interval) {
		System.out.println("in revenue report:SuperAdminService : "+busName);
		List<?> revenue=null;
		if(interval.equalsIgnoreCase("Monthly")) {
			revenue=bookRepo.getMonthlyRevenue(busName);
		}
		else if(interval.equalsIgnoreCase("Quarterly")) {
			revenue=bookRepo.getQuarterlyRevenue(busName);
		}
		else if(interval.equalsIgnoreCase("Yearly")) {
			revenue=bookRepo.getYearlyRevenue(busName);
		}
		 System.out.println("in revenue report:SuperAdminService after : "+busName);
		 return revenue;
	}
	...
	public List<FeedbackDTO> getFeedback(int airId) {
		...
		List<FeedbackDTO> fb=bookRepo.getFeedbackBybusId(airId);
		return fb;
	}
	@Override
	...
	@Override
	public List<BusWiseRevenueDTO> getBusRevenue() {
		return bookRepo.getBusRevenue();
	}
